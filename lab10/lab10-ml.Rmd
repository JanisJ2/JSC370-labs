...
miscla_error
```
We can see that the misclassification error is around 23% which is not bad for a simple tree model.
...
The heart dataset was split into 70% training and 30% testing using a random seed for reproducibility. A classification tree was trained using rpart with minsplit = 10, minbucket = 3, and 10-fold cross-validation. The complexity parameter (cp) table was examined, and the optimal cp was identified as 0.01886792, which minimized the cross-validation error (xerror = 0.4173). The misclassification error was calculated using a confusion matrix, and the final Out-of-Bag (OOB) error for the tree was found to be 41.73%, indicating the expected classification error on unseen data.
...
```{r}
heart_rf <- randomForest(as.factor(AHD) ~ ., data = heart_train, na.action = na.omit)
#oob error rate
